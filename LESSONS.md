...
**2026-07-15 refinement — trip detail landscape notch clipping:** `.mobile-trip-shell` is `position: fixed; inset-x: 0` and previously only handled top safe-area via `.mobile-safe-header`. In landscape the notch becomes a *horizontal* inset, so tab pills (Chat/Calendar/…) and section content (calendar grid, Places Explore pills, etc.) clipped on the left. Fix at the shell with `padding-left/right: env(safe-area-inset-*)` once, then use density classes (`.mobile-trip-tab-pill`, `.mobile-trip-control-pill`, `.mobile-trip-filter-pill`) whose *landscape* overrides are scoped under `.mobile-trip-shell` so shared desktop Places/Polls keep their typography. Avoid stacking `safe-container` / `max(…, env(safe-area-inset-left))` inside the shell — that double-counts the gutter. *Evidence: July 2026 calendar/Places landscape crop report.*

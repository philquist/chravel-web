...
**Never commit `codebase-atlas/index.html` or `codebase-atlas/architecture-data.json`
in a feature branch.** These are generated outputs — committing them in a feature
branch guarantees merge conflicts on every PR because the metrics diverge from whatever
`main` regenerated independently.

Rules:
- `codebase-atlas/curated.json` **may** be committed — it is the hand-authored judgment
  layer and is the only file in `codebase-atlas/` that belongs in a feature branch.
- `codebase-atlas/index.html` and `codebase-atlas/architecture-data.json` must only
  be regenerated and committed on `main` (this is handled automatically by
  `.github/workflows/atlas.yml` after CI passes — do not duplicate that work in a branch).
- If you run `npm run atlas` locally or mid-task to verify the output, do **not** stage
  or commit the resulting `index.html` or `architecture-data.json`. Run
  `git restore codebase-atlas/index.html codebase-atlas/architecture-data.json`
  before committing.
- When the user asks to "refresh the atlas" or "update the atlas" while working in a
  feature branch: update `curated.json` only, then remind the user that the HTML/JSON
  outputs will be auto-regenerated by CI when the branch merges to `main`.

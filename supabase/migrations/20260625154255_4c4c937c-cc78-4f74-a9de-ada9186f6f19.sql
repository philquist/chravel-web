-- Apply PR #750 payment split penny rounding (corrected to use existing is_trip_member helper)
...
BEGIN
  ...
  IF NOT public.is_trip_member(v_auth_uid, p_trip_id) THEN
  ...
  IF EXISTS (
    ...
    WHERE NOT public.is_trip_member(participant_id.user_id::uuid, p_trip_id)
  ) THEN
